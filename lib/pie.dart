...
class Mypie extends StatefulWidget {
  const Mypie({Key? key}) : super(key: key);
...
class _MypieState extends State<Mypie> {
  ...
  Future<void> fetchDataFromAPI() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Failed to fetch data from API. Status code: ${response
            .statusCode}');
      }
    } catch (error) {
  ...
  Widget _buildAccordion(Map<String, dynamic> data) {
    return ExpansionTile(
      ...
      children: [
        ...
        ListTile(
          title: Text(
              'MAD: ${data['MAD']}, COA: ${data['COA']}, IP: ${data['IP']}, WEBX: ${data['WEBX']}'),
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          title: Text('Students marks dasboard'),
          leading: IconButton(
            icon: Icon(Icons.logout),
            onPressed: () {
              Navigator.pop(
                  context); // Navigate back to the previous screen (Admin Login)
            },
          ),
        ),
